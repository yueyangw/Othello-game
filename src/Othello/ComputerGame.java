...
public class ComputerGame extends JPanel implements ActionListener, MouseListener {
    ...
    MachineStep AIPlayer = new MachineStep(difficulty, 8);
    MCTS AIPlayer2 ;
...
//                System.out.println(manType);
                if (nextType == -manType) {
                    ...
                    System.out.println();
                    int[] pos;
                    pos = AIPlayer2.nextStep(board, -manType);
                    ChessBoards.add(change(board, pos[1], pos[0], -manType));

                    nextType = manType;
                    if (!flag(possiblePlace(ChessBoards.get(ChessBoards.size() - 1), manType))) {
                        nextType = -manType;
                    }
                    ...
                    for (int i = 1; i <= 8; i++) {
                        for (int j = 1; j <= 8; j++) {
                            System.out.print(ChessBoards.get(ChessBoards.size() - 1)[j][i] + " ");
                        }
                        System.out.println();
                    }
    ...
    //存棋盘集合
    private ArrayList<int[][]> ChessBoards = new ArrayList<int[][]>();
    ...
    public void createRestart() {
        ...
        restart.addMouseListener(new MouseAdapter() {
            ...
            public void mouseReleased(MouseEvent e) {
                ...
                newGP.AIPlayer2 =new MCTS(difficulty, 8, 10000000, 19260817);

                newGP.blackname = blackname;
                newGP.whitename = whitename;
                newGP.manType = manType;
...
//                showing = false;
    ...
    public void reading(String string) {
        ...
        this.manType = Integer.parseInt(input.nextLine());
